...
int main(int argc, char* argv[]){
    ...
    for(int i=0; i<MAX_THREADS; i++){
        arguments[i].index = i;
        if(i == MAX_THREADS-1 && darts % MAX_THREADS != 0){
            arguments[i].operations = darts - (darts / MAX_THREADS) * i;
        }
        else arguments[i].operations = darts / MAX_THREADS;
